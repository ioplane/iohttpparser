...
static void run_find_bench(const char *backend_name, ihtp_scan_find_fn fn, const find_case_t *test_case,
                           size_t iterations)
{
    ...
    printf("%-8s find  %-18s %12" PRIu64 " ns total  %10.2f ns/op\n", backend_name,
           test_case->name, elapsed_ns, ns_per_iter);
}
                            ...
                            const token_case_t *test_case, size_t iterations)
{
    ...
    printf("%-8s token %-18s %12" PRIu64 " ns total  %10.2f ns/op\n", backend_name,
           test_case->name, elapsed_ns, ns_per_iter);
}
