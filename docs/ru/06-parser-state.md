...
Для производительного контура предпочтителен интерфейс состояния. Интерфейс
без состояния перед делегированием в тот же код разбора каждый раз очищает
выходную структуру, поэтому он проще в применении, но немного дороже в
плотном цикле.
